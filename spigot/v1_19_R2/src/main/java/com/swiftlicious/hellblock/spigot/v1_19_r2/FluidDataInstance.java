...
public class FluidDataInstance implements FluidData {
	...
	private org.bukkit.Fluid minecraftToBukkit(net.minecraft.world.level.material.Fluid minecraft) {
		...
		Frozen registryAccess = minecraftServer.registries().compositeAccess();
		net.minecraft.core.Registry<net.minecraft.world.level.material.Fluid> fluidRegistry = registryAccess
				.registryOrThrow(Registries.FLUID);
		org.bukkit.Fluid bukkit = Registry.FLUID.get(
				CraftNamespacedKey.fromMinecraft(fluidRegistry.getResourceKey(minecraft).orElseThrow().location()));
