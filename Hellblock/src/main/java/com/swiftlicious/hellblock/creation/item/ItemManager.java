...
public class ItemManager implements ItemManagerInterface, Listener {
	...
	public boolean isLavaFishingItem(ItemStack itemStack) {
		if (itemStack == null || itemStack.getType() == Material.AIR)
			return false;
		return NBT.get(itemStack, nbtItem -> nbtItem.hasTag("LavaFishing")
				&& !nbtItem.getCompound("LavaFishing").getString("id").equals(""));
	}
	...
	public String getLavaFishingItemID(ItemStack itemStack) {
		...
		if (!isLavaFishingItem(itemStack))
			return null;
		return NBT.get(itemStack, nbtItem -> {
			ReadableNBT hbCompound = nbtItem.getCompound("LavaFishing");
			if (hbCompound == null)
				return null;
			return hbCompound.getString("id");
		});
	}
	...
	public HBBuilder getItemBuilder(ConfigurationSection section, String type, String id) {
		...
		itemHBBuilder.stackable(section.getBoolean("stackable", true))
				...
				.itemFlag(section.getStringList("item-flags").stream().map(flag -> ItemFlag.valueOf(flag.toUpperCase()))
						.toList())
				.enchantment(
	...
	public ItemStack build(Player player, ItemBuilder builder, Map<String, String> placeholders) {
		...
		return NBT.modify(temp, nbtItem -> {
	...
	public static class HBBuilder implements ItemBuilder {
		...
		public ItemBuilder customModelData(int value) {
			if (value == 0)
				return this;
			editors.put("custom-model-data",
					(player, nbtItem, placeholders) -> nbtItem.setInteger("CustomModelData", value));
			return this;
		}
		...
		public ItemBuilder name(String name) {
			...
			editors.put("name", (player, nbtItem, placeholders) -> {
				ReadWriteNBT displayCompound = nbtItem.getOrCreateCompound("display");
				displayCompound.setString("Name",
		...
		public ItemBuilder tag(boolean tag, String type, String id) {
			editors.put("tag", (player, nbtItem, placeholders) -> {
				if (!tag)
					return;
				ReadWriteNBT hbCompound = nbtItem.getOrCreateCompound("LavaFishing");
				hbCompound.setString("type", type);
				hbCompound.setString("id", id);
			});
			return this;
		}
		...
		public ItemBuilder lore(List<String> lore) {
			...
			editors.put("lore", (player, nbtItem, placeholders) -> {
				ReadWriteNBT displayCompound = nbtItem.getOrCreateCompound("display");
				ReadWriteNBTList<String> list = displayCompound.getStringList("Lore");
				list.clear();
				list.addAll(lore
						.stream().map(
		...
		public ItemBuilder nbt(Map<String, Object> nbt) {
			if (nbt.size() == 0)
				return this;
			editors.put("nbt", (player, nbtItem, placeholders) -> NBTUtils.setTagsFromBukkitYAML(player, placeholders,
					nbtItem, nbt));
			return this;
		}
		...
		public ItemBuilder nbt(ConfigurationSection section) {
			if (section == null)
				return this;
			editors.put("nbt", (player, nbtItem, placeholders) -> NBTUtils.setTagsFromBukkitYAML(player, placeholders,
					nbtItem, section.getValues(false)));
			return this;
		}
		...
		public ItemBuilder enchantment(List<Pair<String, Short>> enchantments, boolean store) {
			...
			editors.put("enchantment", (player, nbtItem, placeholders) -> {
				ReadWriteNBTCompoundList list = nbtItem.getCompoundList(store ? "StoredEnchantments" : "Enchantments");
				for (Pair<String, Short> pair : enchantments) {
					ReadWriteNBT nbtCompound = list.addCompound();
			...
			editors.put("random-enchantment", (player, nbtItem, placeholders) -> {
				...
				for (Tuple<Double, String, Short> pair : enchantments) {
					if (Math.random() < pair.left() && !ids.contains(pair.mid())) {
						ReadWriteNBT nbtCompound = list.addCompound();
						nbtCompound.setString("id", pair.mid());
						nbtCompound.setShort("lvl", pair.right());
						ids.add(pair.mid());
					}
				}
			...
			editors.put("enchantment-pool", (player, nbtItem, placeholders) -> {
				...
				outer: while (i < amount && cloned.size() != 0) {
					...
					Enchantment enchantment = enchantmentRegistry
							.getOrThrow(NamespacedKey.fromString(enchantPair.left()));
					if (enchantment == null) {
		...
		public ItemBuilder maxDurability(int max) {
			...
			editors.put("durability", (player, nbtItem, placeholders) -> {
				ReadWriteNBT hbCompound = nbtItem.getOrCreateCompound("LavaFishing");
				hbCompound.setInteger("max_dur", max);
				hbCompound.setInteger("cur_dur", max);
			});
			return this;
		}
		...
		public ItemBuilder size(Pair<Float, Float> size) {
			...
			editors.put("size", (player, nbtItem, placeholders) -> {
				...
				hbCompound.setFloat("size", random);
		...
		public ItemBuilder stackable(boolean stackable) {
			...
			editors.put("stackable", (player, nbtItem, placeholders) -> {
				ReadWriteNBT hbCompound = nbtItem.getOrCreateCompound("LavaFishing");
				hbCompound.setUUID("uuid", UUID.randomUUID());
			});
			return this;
		}
		...
		public ItemBuilder head(String base64) {
			if (base64 == null)
				return this;
			return (ItemBuilder) editors.put("head", (player, nbtItem, placeholders) -> {
				ReadWriteNBT nbtCompound = nbtItem.getOrCreateCompound("SkullOwner");
				nbtCompound.setUUID("Id", UUID.nameUUIDFromBytes(id.getBytes()));
				nbtCompound.getOrCreateCompound("Properties").getCompoundList("textures").addCompound()
		...
		public ItemBuilder randomDamage(boolean damage) {
			...
			editors.put("damage", (player, nbtItem, placeholders) -> {
				if (!nbtItem.hasTag("LavaFishing"))
					return;
				ReadWriteNBT hbCompound = nbtItem.getCompound("LavaFishing");
				if (hbCompound != null) {
					int i = hbCompound.getInteger("max_dur");
					if (i != 0) {
						int dur = ThreadLocalRandom.current().nextInt(i);
						hbCompound.setInteger("cur_dur", dur);
						nbtItem.setInteger("Damage", (int) (NBT.itemStackFromNBT(nbtItem).getType().getMaxDurability()
								* ((double) dur / i)));
					} else {
						nbtItem.setInteger("Damage", ThreadLocalRandom.current()
								.nextInt(NBT.itemStackFromNBT(nbtItem).getType().getMaxDurability()));
					}
				} else {
					nbtItem.setInteger("Damage", ThreadLocalRandom.current()
							.nextInt(NBT.itemStackFromNBT(nbtItem).getType().getMaxDurability()));
	...
	public void onPickUp(EntityPickupItemEvent event) {
		...
		NBT.modify(itemStack, nbtItem -> {
			if (!nbtItem.hasTag("owner"))
				return;
			if (!Objects.equals(nbtItem.getString("owner"), player.getName())) {
	...
	public void onRepairItem(PrepareAnvilEvent event) {
		...
		if (!isLavaFishingItem(result))
			return;
		NBT.modify(result, nbtItem -> {
			ReadWriteNBT compound = nbtItem.getCompound("LavaFishing");
			...
			compound.setInteger("cur_dur",
					(int) (max_dur * (1 - (double) damageable.getDamage() / result.getType().getMaxDurability())));
			event.setResult(NBT.itemStackFromNBT(nbtItem));
		});
	}
	...
	public void onMending(PlayerItemMendEvent event) {
		...
		NBT.get(itemStack, nbtItem -> {
			ReadableNBT compound = nbtItem.getCompound("LavaFishing");
			if (compound == null)
				return;
			event.setCancelled(true);
