...
public class AdventureManager implements AdventureManagerInterface {
	...
	public void sendMessageWithPrefix(CommandSender sender, String s) {
		if (s == null)
			return;
		if (sender == null)
			sendGlobalMessage("<dark_red>[Hellblock] " + s);
		else if (sender instanceof Player player)
			sendPlayerMessage(player, "<dark_red>[Hellblock] " + s);
		else if (sender instanceof ConsoleCommandSender)
			sendConsoleMessage("<dark_red>[Hellblock] " + s);
	}
