...
import com.swiftlicious.hellblock.gui.MarketManager;
...
public class HellblockPlugin extends JavaPlugin {
	...
	public void onLoad() {
		if (!CommandAPI.isLoaded())
			CommandAPI.onLoad(new CommandAPIBukkitConfig(this).usePluginNamespace().shouldHookPaperReload(true).silentLogs(true).verboseOutput(true));
		...
		this.dependencyManager.loadDependencies(new ArrayList<>(
				List.of(Dependency.GSON, Dependency.SLF4J_API, Dependency.SLF4J_SIMPLE, Dependency.COMMAND_API,
						Dependency.BOOSTED_YAML, Dependency.NBT_API, Dependency.EXP4J, Dependency.MYSQL_DRIVER,
	...
	public void onEnable() {
		if (!NBT.preloadApi()) {
			LogUtils.severe("NBT-API wasn't initialized properly, disabling Hellblock.");
		...
		if (!isHookedPluginEnabled("ProtocolLib")) {
			LogUtils.severe(
					"ProtocolLib is a dependency for Hellblock to function properly. Please install this on the server.");
			Bukkit.getPluginManager().disablePlugin(this);
			return;
		}

		if (!CommandAPI.isLoaded()) {
			LogUtils.severe("CommandAPI wasn't initialized properly, disabling Hellblock.");
			Bukkit.getPluginManager().disablePlugin(this);
			return;
		}
		...
		if (HBConfig.updateChecker) {
			this.versionManager.checkUpdate().thenAccept(result -> {
				if (!result) {
					getAdventureManager().sendConsoleMessage("[Hellblock] You are using the latest version.");
				} else {
					getAdventureManager().sendConsoleMessage(
							"[Hellblock] Update is available: <u>https://github.com/Swiftlicious01/Hellblock<!u>");
		...
		getAdventureManager().sendConsoleMessage("Took " + time3 + " seconds to setup Hellblock!");
		...
		getScheduler().runTaskAsyncTimer(() -> getLavaRain().startLavaRainProcess(), 5, 10, TimeUnit.MINUTES);
		getHellblockHandler().getHellblockWorld();
		getNetherBrewing().addRecipe();
	}
	...
	public void onDisable() {
		...
		if (this.adventureManager != null)
			((AdventureManager) this.adventureManager).close();
