...
public class GenMode {
	...
	private XParticle particleEffect = null;
	...
	public GenMode(int id, List<Material> blocks, Map<BlockFace, Material> fixedBlocks, boolean searchForPlayersNearby, String name, Material fallbackMaterial) {
		...
		if (name != null && !name.trim().equals("")) {
			...
			this.setName(sjName);
			LogUtils.warn("No displayname found for generator with id: " + id + " - Created name: " + sjName);
		}
		valid = true;
	}
	...
	public void setGenSound(Sound genSound) {
		if (genSound == null) {
			LogUtils.warn("Unkown sound for generator with id: " + this.getId());
			return;
		}
	...
	public void setParticleEffect(XParticle particleEffect) {
...
//		loc = loc.add(0.5D, 1D, 0.5D);
		for (int i = 0; i < 10; i++) {
			...
			float speed = 1 / (i + 1);
			ParticleDisplay.of(this.particleEffect).withLocation(tempLoc).withCount((int) speed).spawn();
		}
	}
