...
public class DebugCommand {
	...
	public CommandAPICommand getDebugCommand() {
		return new CommandAPICommand("debug").withSubcommands(getLootChanceCommand(), getGroupCommand(),
				getNBTCommand(), getLocationCommand());
	}
	...
	public CommandAPICommand getNBTCommand() {
		return new CommandAPICommand("nbt").executesPlayer((player, arg) -> {
			...
			HellblockPlugin.getInstance().getConfigUtils()
					.mapToReadableStringList(NBTUtils.compoundToMap(new RtagItem(item)), list, 0, false);
	...
	public CommandAPICommand getLootChanceCommand() {
		return new CommandAPICommand("loot-chance")
				...
				.executesPlayer((player, arg) -> {
					...
					boolean inLava = (boolean) arg.getOrDefault("lava fishing", true);
					fishingPreparation.insertArg("{lava}", String.valueOf(inLava));
					fishingPreparation.mergeEffect(initialEffect);
					var map = HellblockPlugin.getInstance().getLootManager()
							.getPossibleLootKeysWithWeight(initialEffect, fishingPreparation);
