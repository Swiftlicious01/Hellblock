...
public class DynamicText {
	...
	private void analyze(String value) {
		...
		List<String> placeholdersOwner = new ArrayList<>(
				HellblockPlugin.getInstance().getPlaceholderManager().detectPlaceholders(value));
		String origin = value;
	...
	public boolean update(Map<String, String> placeholders) {
		...
		if (this.placeholders.length != 0) {
			...
			if ("%s".equals(originalValue)) {
				string = placeholderManager.getSingleValue(owner, this.placeholders[0], placeholders);
			} else {
				Object[] values = new String[this.placeholders.length];
				for (int i = 0; i < this.placeholders.length; i++) {
					values[i] = placeholderManager.getSingleValue(owner, this.placeholders[i], placeholders);
				}
