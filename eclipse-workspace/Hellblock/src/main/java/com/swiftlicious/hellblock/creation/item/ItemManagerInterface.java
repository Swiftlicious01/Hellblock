...
public interface ItemManagerInterface {
	 ...
	 *
	 * @param player The player for whom the ItemStack is being built.
	 * @param id     The item ID, which include an identification (e.g.,
	 *               "Oraxen:id").
	 ...
	 * @return The item ID without type or null if not found or if the ItemStack is
	 *         null or empty.
	 */
	@Nullable
	String getLavaFishingItemID(ItemStack itemStack);
	...
	/**
	 * Drops an item based on the provided loot, applying velocity from a hook
	 * location to a player location.
	 *
	 * @param player         The player for whom the item is intended.
	 * @param hookLocation   The location where the item will initially drop.
	 * @param playerLocation The target location towards which the item's velocity
	 *                       is applied.
	 * @param itemStack      The loot to drop
	 * @param condition      A map of placeholders for item customization.
	 */
	void dropItem(Player player, Location hookLocation, Location playerLocation, ItemStack itemStack,
			Condition condition);
	 ...
	 */
	boolean isLavaFishingItem(ItemStack itemStack);
