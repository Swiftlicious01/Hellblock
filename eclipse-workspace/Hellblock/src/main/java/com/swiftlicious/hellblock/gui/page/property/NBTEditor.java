...
public class NBTEditor {
	...
	public List<Item> getNBTContents() {
		...
		for (Map.Entry<String, Object> entry : currentSection.getValues(false).entrySet()) {
			...
			if (entry.getValue() instanceof @SuppressWarnings("unused") List<?> list) {
			...
			} else if (entry.getValue() instanceof @SuppressWarnings("unused") Map<?, ?> map) {
	...
	public void reOpenMain() {
		Gui upperGui = Gui.normal().setStructure("b b c").addIngredient('b', new ItemStack(Material.AIR))
				.addIngredient('c', new SaveIcon()).build();
		...
		var gui = PagedGui.items()
				...
				.addIngredient('#', new BackGroundItem()).setContent(getNBTContents()).build();
		...
		var window = AnvilWindow.split().setViewer(player)
				.setTitle(new ShadedAdventureComponentWrapper(HellblockPlugin.getInstance().getAdventureManager()
						.getComponentFromMiniMessage(HBLocale.GUI_NBT_EDIT_TITLE)))
	...
	public void reOpenAddCompound() {
		...
		Gui upperGui = Gui.normal().setStructure("a b c")
				.addIngredient('a', new ItemBuilder(Material.COMMAND_BLOCK_MINECART).setDisplayName(""))
				.addIngredient('b', new ItemStack(Material.AIR)).addIngredient('c', confirm).build();
		...
		var window = AnvilWindow
				.split().setViewer(player).setTitle(new ShadedAdventureComponentWrapper(HellblockPlugin.getInstance()
						.getAdventureManager().getComponentFromMiniMessage(HBLocale.GUI_TITLE_NBT_COMPOUND)))
				.addRenameHandler(s -> {
					value = s;
	...
	public void reOpenAddList() {
		...
		var window = AnvilWindow
				.split().setViewer(player).setTitle(new ShadedAdventureComponentWrapper(HellblockPlugin.getInstance()
						.getAdventureManager().getComponentFromMiniMessage(HBLocale.GUI_TITLE_NBT_LIST)))
				.addRenameHandler(s -> {
					value = s;
					confirm.notifyWindows();
				}).setUpperGui(upperGui).setLowerGui(gui).build();
	...
	public void reOpenAddValue() {
		...
		var window = AnvilWindow
				.split().setViewer(player).setTitle(new ShadedAdventureComponentWrapper(HellblockPlugin.getInstance()
						.getAdventureManager().getComponentFromMiniMessage(HBLocale.GUI_TITLE_NBT_KEY)))
				.addRenameHandler(s -> {
					value = s;
					confirm.notifyWindows();
				}).setUpperGui(upperGui).setLowerGui(gui).build();
	...
	public void reOpenSetValue(String key, String type) {
		...
		Gui upperGui = Gui.normal().setStructure("a b c")
				.addIngredient('a',
						new ItemBuilder(Material.COMMAND_BLOCK).setDisplayName(type == null ? "" : "(" + type + ") "))
				.addIngredient('b', new ItemStack(Material.AIR)).addIngredient('c', save).build();
		...
		var gui = PagedGui.items()
				.setStructure("x x x x x x x x x", "x x x x x x x x x", "x x x x x x x x x", "# # # # c # # # #")
				.addIngredient('x', Markers.CONTENT_LIST_SLOT_HORIZONTAL).addIngredient('c', parentPage.getBackItem())
				.addIngredient('#', new BackGroundItem()).setContent(getTypeContents(key)).build();

		var window = AnvilWindow
				.split().setViewer(player).setTitle(new ShadedAdventureComponentWrapper(HellblockPlugin.getInstance()
						.getAdventureManager().getComponentFromMiniMessage(HBLocale.GUI_NBT_SET_VALUE_TITLE)))
				.addRenameHandler(s -> {
					value = s;
					save.notifyWindows();
				}).setUpperGui(upperGui).setLowerGui(gui).build();
	...
	public List<Item> getTypeContents(String key) {
		...
		for (Map.Entry<String, String> entry : Map.of("String", "some text", "Byte", "1", "Short", "123", "Int",
				"123456", "Long", "123456789", "Double", "1.2345", "Float", "1.23", "Boolean", "true", "IntArray",
				"[111,222,333,444]", "ByteArray", "[1,2,3,4]").entrySet()) {
	...
	public class TypeItem extends AbstractItem {
		...
		public ItemProvider getItemProvider() {
			return new ItemBuilder(Material.BELL).setDisplayName(new ShadedAdventureComponentWrapper(HellblockPlugin
					.getInstance().getAdventureManager().getComponentFromMiniMessage("(" + type + ") " + tip)));
		}
			...
			if (value == null || value.equals("") || value.contains(".") || currentSection.contains(value)) {
				return new ItemBuilder(Material.BARRIER)
						.setDisplayName(new ShadedAdventureComponentWrapper(HellblockPlugin.getInstance()
								.getAdventureManager().getComponentFromMiniMessage(HBLocale.GUI_NBT_INVALID_KEY)));
			}

			return new ItemBuilder(Material.COMMAND_BLOCK_MINECART)
					.setDisplayName(new ShadedAdventureComponentWrapper(HellblockPlugin.getInstance()
							.getAdventureManager().getComponentFromMiniMessage(HBLocale.GUI_NEW_VALUE + value)))
	...
	public class NewCompoundIcon extends AbstractItem {
		...
		@Override
		public ItemProvider getItemProvider() {
			return new ItemBuilder(Material.OAK_SIGN).setDisplayName(new ShadedAdventureComponentWrapper(HellblockPlugin
					.getInstance().getAdventureManager().getComponentFromMiniMessage(HBLocale.GUI_NBT_ADD_COMPOUND)));
		}
	...
	public class NewListIcon extends AbstractItem {
		...
		public ItemProvider getItemProvider() {
			return new ItemBuilder(Material.SPRUCE_SIGN)
					.setDisplayName(new ShadedAdventureComponentWrapper(HellblockPlugin.getInstance()
							.getAdventureManager().getComponentFromMiniMessage(HBLocale.GUI_NBT_ADD_LIST)));
		}
	...
	public class NewValueIcon extends AbstractItem {
		...
		@Override
		public ItemProvider getItemProvider() {
			return new ItemBuilder(Material.ACACIA_SIGN)
					.setDisplayName(new ShadedAdventureComponentWrapper(HellblockPlugin.getInstance()
							.getAdventureManager().getComponentFromMiniMessage(HBLocale.GUI_NBT_ADD_VALUE)));
		}
			...
			return new ItemBuilder(Material.COMMAND_BLOCK_MINECART)
					.setDisplayName(
							new ShadedAdventureComponentWrapper(HellblockPlugin.getInstance().getAdventureManager()
									.getComponentFromMiniMessage("Compound: " + splits[splits.length - 1])))
					.addLoreLines("")
					.addLoreLines(new ShadedAdventureComponentWrapper(HellblockPlugin.getInstance()
	...
	public class InvValueIcon extends AbstractItem {
		...
		public ItemProvider getItemProvider() {
			...
			return new ItemBuilder(Material.REPEATING_COMMAND_BLOCK)
					.setDisplayName(
							new ShadedAdventureComponentWrapper(HellblockPlugin.getInstance().getAdventureManager()
									.getComponentFromMiniMessage(splits[splits.length - 1] + ": " + value)))
					.addLoreLines("")
					.addLoreLines(new ShadedAdventureComponentWrapper(HellblockPlugin.getInstance()
							.getAdventureManager().getComponentFromMiniMessage(HBLocale.GUI_LEFT_CLICK_EDIT)))
					.addLoreLines(new ShadedAdventureComponentWrapper(HellblockPlugin.getInstance()
	...
	public class InvListIcon extends AbstractItem {
		...
		public ItemProvider getItemProvider() {
			...
			return new ItemBuilder(Material.CHAIN_COMMAND_BLOCK)
					.setDisplayName(new ShadedAdventureComponentWrapper(HellblockPlugin.getInstance()
							.getAdventureManager().getComponentFromMiniMessage("List: " + splits[splits.length - 1])))
					.addLoreLines("")
					.addLoreLines(
							new ShadedAdventureComponentWrapper(HellblockPlugin.getInstance().getAdventureManager()
									.getComponentFromMiniMessage("<st>" + HBLocale.GUI_LEFT_CLICK_EDIT + "</st>")))
					.addLoreLines(new ShadedAdventureComponentWrapper(HellblockPlugin.getInstance()
							.getAdventureManager().getComponentFromMiniMessage(HBLocale.GUI_RIGHT_CLICK_DELETE)));
		}
	...
	public class InvMapIcon extends AbstractItem {
		...
		public ItemProvider getItemProvider() {
			...
			return new ItemBuilder(Material.COMMAND_BLOCK)
					.setDisplayName(new ShadedAdventureComponentWrapper(HellblockPlugin.getInstance()
							.getAdventureManager().getComponentFromMiniMessage("Map: " + splits[splits.length - 1])))
					.addLoreLines("")
					.addLoreLines(
							new ShadedAdventureComponentWrapper(HellblockPlugin.getInstance().getAdventureManager()
									.getComponentFromMiniMessage("<st>" + HBLocale.GUI_LEFT_CLICK_EDIT + "</st>")))
	...
	public class SaveValueIcon extends AbstractItem {
		...
		public ItemProvider getItemProvider() {
			try {
				...
				return new ItemBuilder(Material.COMMAND_BLOCK).setDisplayName(value)
						.addLoreLines(new ShadedAdventureComponentWrapper(HellblockPlugin.getInstance()
								.getAdventureManager().getComponentFromMiniMessage(HBLocale.GUI_CLICK_CONFIRM)))
						.addLoreLines(new ShadedAdventureComponentWrapper(HellblockPlugin.getInstance()
								.getAdventureManager().getComponentFromMiniMessage(HBLocale.GUI_RIGHT_CLICK_CANCEL)));
				...
				return new ItemBuilder(Material.BARRIER)
						.setDisplayName(new ShadedAdventureComponentWrapper(HellblockPlugin.getInstance()
								.getAdventureManager().getComponentFromMiniMessage(HBLocale.GUI_ILLEGAL_FORMAT)));
			}
		}
	...
	public class SaveIcon extends AbstractItem {
		...
		public ItemProvider getItemProvider() {
			if (nbtSection.getValues(false).size() > 0) {
				var builder = new ItemBuilder(Material.ACACIA_SIGN)
						.setDisplayName(new ShadedAdventureComponentWrapper(HellblockPlugin.getInstance()
								.getAdventureManager().getComponentFromMiniMessage(HBLocale.GUI_NBT_PREVIEW)));
				for (String line : HellblockPlugin.getInstance().getConfigUtils()
						.getReadableSection(nbtSection.getValues(false))) {
					builder.addLoreLines(new ShadedAdventureComponentWrapper(
	...
	public class BackToParentIcon extends AbstractItem {
		...
		public ItemProvider getItemProvider() {
			return new ItemBuilder(Material.MINECART)
					.setDisplayName(new ShadedAdventureComponentWrapper(HellblockPlugin.getInstance()
							.getAdventureManager().getComponentFromMiniMessage(HBLocale.GUI_NBT_BACK_TO_COMPOUND)));
		}
		...
		public void handleClick(@NotNull ClickType clickType, @NotNull Player player,
			...
			currentNode = currentNode.lastIndexOf(".") == -1 ? ""
					: currentNode.substring(0, currentNode.lastIndexOf("."));
			reOpenMain();
		}
	}
}
