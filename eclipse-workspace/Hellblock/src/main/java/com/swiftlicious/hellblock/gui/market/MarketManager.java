...
public class MarketManager implements MarketManagerInterface, Listener {
	...
	public boolean checkMarketPrice(RtagItem tag) {
		if (tag == null || tag.get("HellMarket", 0) == null)
			return false;

		boolean data = false;
		if (tag.get("HellMarket", 0) instanceof HashMap) {
			HashMap<String, Object> map = tag.get("HellMarket", 0);
	...
	public double getMarketPrice(RtagItem tag) {
		if (tag == null || tag.get("HellMarket", 0) == null)
			return 0.0D;
	...
	public @Nullable ItemStack setMarketPrice(RtagItem tag, double price) {
		if (tag == null)
			return null;

		if (tag.get("HellMarket", 0) == null) {
			if (tag.get() != null) {
				tag.add(Map.of("HellMarket", List.of(Map.of("price", price))), tag.get());
			} else {
				tag.add(Map.of("HellMarket", List.of(Map.of("price", price))));
			}
			return tag.load();
		}
			...
			for (String key : map.keySet()) {
				if (key.equals("price")) {
					for (Object value : map.values()) {
						if (value instanceof Double) {
							value = price;
							tag.set(List.of(Map.of("price", value)), map);
						}
					}
				}
			}
	...
	public @Nullable ItemStack setSize(RtagItem tag, float size) {
		if (tag == null)
			return null;

		if (tag.get("HellMarket", 1) == null) {
			if (tag.get() != null) {
				tag.add(Map.of("HellMarket", List.of(Map.of("size", size))), tag.get());
			} else {
				tag.add(Map.of("HellMarket", List.of(Map.of("size", size))));
			}
			return tag.load();
		}

		if (tag.get("HellMarket", 1) instanceof HashMap) {
			HashMap<String, Object> map = tag.get("HellMarket", 1);
			for (String key : map.keySet()) {
				if (key.equals("size")) {
					for (Object value : map.values()) {
						if (value instanceof Float) {
							value = size;
							tag.set(List.of(Map.of("size", value)), map);
						}
					}
				}
			}
	...
	public @Nullable ItemStack setUUID(RtagItem tag, @Nullable UUID uuid) {
		if (tag == null || uuid == null)
			return null;

		if (tag.get("HellMarket", 2) == null) {
			if (tag.get() != null) {
				tag.add(Map.of("HellMarket", List.of(Map.of("uuid", uuid))), tag.get());
			} else {
				tag.add(Map.of("HellMarket", List.of(Map.of("uuid", uuid))));
			}
			return tag.load();
		}

		if (tag.get("HellMarket", 2) instanceof HashMap) {
			HashMap<String, Object> map = tag.get("HellMarket", 2);
			for (String key : map.keySet()) {
				if (key.equals("uuid")) {
					for (Object value : map.values()) {
						if (value instanceof UUID) {
							value = uuid;
							tag.set(List.of(Map.of("uuid", value)), map);
						}
					}
				}
			}
	...
	public double getItemPrice(ItemStack itemStack) {
		...
		if (checkMarketPrice(tagItem)) {
			Double price = getMarketPrice(tagItem);
		...
		if (NBTUtils.hasNBTItemComponentData(itemStack, "custom_model_data")) {
			itemID = itemID + ":" + NBTUtils.getNBTItemComponentData(itemStack, "custom_model_data");
		}
