...
public class MarketGUI {
	 ...
	 *
	 * @param manager     The Market Manager implementation associated with this
	 *                    MarketGUI.
	 * @param player      The player who owns this MarketGUI.
	 * @param earningData Data related to earnings for this MarketGUI.
	 */
	public MarketGUI(MarketManager manager, Player player, EarningData earningData) {
	...
	public MarketGUI refresh() {
		...
		if (sellElement != null && sellElement.getSlots().size() > 0) {
			...
			if (totalWorth <= 0) {
				sellElement.setItemStack(manager.getSellIconDenyBuilder().build(owner,
		...
		if (sellAllElement != null && sellAllElement.getSlots().size() > 0) {
			...
			if (totalWorth <= 0) {
				sellAllElement.setItemStack(manager.getSellAllIconDenyBuilder().build(owner,
						Map.of("{money}", HellblockPlugin.getInstance().getNumberUtils().money(totalWorth),
								"{money_formatted}", String.format("%.2f", totalWorth), "{player}", owner.getName(),
								"{rest}",
								HellblockPlugin.getInstance().getNumberUtils()
