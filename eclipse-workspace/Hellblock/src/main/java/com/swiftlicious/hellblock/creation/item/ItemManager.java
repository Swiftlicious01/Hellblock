...
public class ItemManager implements ItemManagerInterface, Listener {
	...
	/**
	 * Get a set of all item keys in the LavaFishing plugin.
	 *
	...
	public boolean isLavaFishingItem(ItemStack itemStack) {
		...
		return instance.getFishingManager().checkFishingID(tagItem)
				&& (!instance.getFishingManager().getFishingID(tagItem).equals(""));
	}
	...
	public String getLavaFishingItemID(ItemStack itemStack) {
		...
		if (!isLavaFishingItem(itemStack))
			return null;
		RtagItem tagItem = new RtagItem(itemStack);
		return (String) instance.getFishingManager().getFishingID(tagItem);
	}
	...
	public HBBuilder getItemBuilder(ConfigurationSection section, String type, String id) {
		...
		itemHBBuilder.stackable(section.getBoolean("stackable", true))
				...
				.customModelData(section.getInt("custom-model-data")).nbt(section.getConfigurationSection("nbt"))
	...
	public ItemStack build(Player player, ItemBuilder builder, Map<String, String> placeholders) {
		...
		RtagItem tagItem = new RtagItem(temp);
		for (ItemBuilder.ItemPropertyEditor editor : builder.getEditors()) {
			editor.edit(player, tagItem, placeholders);
		}
	...
	public static class HBBuilder implements ItemBuilder {
		...
		public ItemBuilder customModelData(int value) {
			if (value == 0)
				return this;
			editors.put("custom-model-data", (player, tagItem, placeholders) -> {
				tagItem.set("minecraft:custom_model_data", value);
				tagItem.load();
				tagItem.update();
			});
			return this;
		...
		public ItemBuilder name(String name) {
			if (name == null)
				return this;
			editors.put("name", (player, tagItem, placeholders) -> {
		...
		public ItemBuilder tag(boolean tag, String type, String id) {
			editors.put("tag", (player, tagItem, placeholders) -> {
				if (!tag)
					return;
				HellblockPlugin.getInstance().getFishingManager().setFishingType(tagItem, type);
				HellblockPlugin.getInstance().getFishingManager().setFishingID(tagItem, id);
			});
			return this;
		}
		...
		public ItemBuilder unbreakable(boolean unbreakable) {
			editors.put("unbreakable", (player, tagItem, placeholders) -> {
				if (!unbreakable)
					return;
				tagItem.set("minecraft:unbreakable", unbreakable);
				tagItem.load();
				tagItem.update();
			});
			return this;
		...
		public ItemBuilder lore(List<String> lore) {
			if (lore.size() == 0)
				return this;
			editors.put("lore", (player, tagItem, placeholders) -> {
		...
		public ItemBuilder nbt(Map<String, Object> nbt) {
			if (nbt.size() == 0)
				return this;
			editors.put("nbt", (player, tagItem, placeholders) -> NBTUtils.setTagsFromBukkitYAML(player, placeholders,
					tagItem, nbt));
			return this;
		}
		...
		public ItemBuilder nbt(ConfigurationSection section) {
			if (section == null)
				return this;
			editors.put("nbt", (player, tagItem, placeholders) -> NBTUtils.setTagsFromBukkitYAML(player, placeholders,
					tagItem, section.getValues(false)));
			return this;
		}
		...
		public ItemBuilder enchantment(List<Pair<String, Short>> enchantments, boolean store) {
			if (enchantments.size() == 0)
				return this;
			editors.put("enchantment", (player, tagItem, placeholders) -> {
		...
		public ItemBuilder randomEnchantments(List<Tuple<Double, String, Short>> enchantments, boolean store) {
			if (enchantments.size() == 0)
				return this;
			editors.put("random-enchantment", (player, tagItem, placeholders) -> {
		...
		public ItemBuilder enchantmentPool(List<Pair<Integer, Value>> amountPairs,
			...
			if (enchantments.size() == 0 || amountPairs.size() == 0)
				return this;
			editors.put("enchantment-pool", (player, tagItem, placeholders) -> {
		...
		public ItemBuilder maxDurability(int max) {
			if (max == 0)
				return this;
			editors.put("durability", (player, tagItem, placeholders) -> {
				HellblockPlugin.getInstance().getFishingManager().setMaxDurability(tagItem, max);
				HellblockPlugin.getInstance().getFishingManager().setCurrentDurability(tagItem, max);
			});
			return this;
		}
		...
		public ItemBuilder price(float base, float bonus) {
			if (base == 0 && bonus == 0)
				return this;
			editors.put("price", (player, tagItem, placeholders) -> {
				...
				HellblockPlugin.getInstance().getMarketManager().setMarketPrice(tagItem, price);
		...
		public ItemBuilder size(Pair<Float, Float> size) {
			if (size == null)
				return this;
			editors.put("size", (player, tagItem, placeholders) -> {
				...
				HellblockPlugin.getInstance().getMarketManager().setSize(tagItem, random);
		...
		public ItemBuilder stackable(boolean stackable) {
			if (stackable)
				return this;
			editors.put("stackable", (player, tagItem, placeholders) -> {
				HellblockPlugin.getInstance().getMarketManager().setUUID(tagItem, UUID.randomUUID());
			});
			return this;
		}
		...
		public ItemBuilder preventGrabbing(boolean prevent) {
			if (!prevent)
				return this;
			editors.put("grabbing", (player, tagItem, placeholders) -> {
				ItemUtils.setOwnerData(tagItem, placeholders.get("player"));
			});
			return this;
		}
		...
		public ItemBuilder head(String base64) {
			if (base64 == null)
				return this;
			return (ItemBuilder) editors.put("head", (player, tagItem, placeholders) -> {
		...
		public ItemBuilder randomDamage(boolean damage) {
			if (!damage)
				return this;
			editors.put("damage", (player, tagItem, placeholders) -> {
				if (HellblockPlugin.getInstance().getFishingManager().checkMaxDurability(tagItem)) {
					int i = HellblockPlugin.getInstance().getFishingManager().getMaxDurability(tagItem);
					if (i != 0) {
						int dur = ThreadLocalRandom.current().nextInt(i);
						HellblockPlugin.getInstance().getFishingManager().setCurrentDurability(tagItem, dur);
						tagItem.setComponent("minecraft:damage",
								(int) (tagItem.load().getType().getMaxDurability() * ((double) dur / i)));
						tagItem.load();
						tagItem.update();
					} else {
						tagItem.setComponent("minecraft:damage",
								ThreadLocalRandom.current().nextInt(tagItem.load().getType().getMaxDurability()));
						tagItem.load();
						tagItem.update();
					}
				} else {
	...
	public void onPickUp(PlayerAttemptPickupItemEvent event) {
		...
		RtagItem tagItem = new RtagItem(itemStack);
		if (!ItemUtils.checkOwnerData(tagItem))
			return;
		if (!Objects.equals(ItemUtils.getOwnerData(tagItem), player.getName())) {
			event.setCancelled(true);
		} else {
			ItemUtils.removeOwnerData(tagItem);
	...
	public void onRepairItem(PrepareAnvilEvent event) {
		...
		if (!instance.getFishingManager().checkMaxDurability(tagItem))
			return;
		if (!(result.getItemMeta() instanceof Damageable damageable)) {
			return;
		}
		int maxDur = instance.getFishingManager().getMaxDurability(tagItem);
		instance.getFishingManager().setCurrentDurability(tagItem,
