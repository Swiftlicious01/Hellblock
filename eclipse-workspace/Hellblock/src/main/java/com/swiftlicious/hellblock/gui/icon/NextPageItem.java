		...
		builder.setDisplayName(
				new ShadedAdventureComponentWrapper(
						HellblockPlugin.getInstance().getAdventureManager()
								.getComponentFromMiniMessage(HBLocale.GUI_NEXT_PAGE)))
				.addLoreLines(new ShadedAdventureComponentWrapper(HellblockPlugin.getInstance().getAdventureManager()
						.getComponentFromMiniMessage(gui.hasNextPage()
								? HBLocale.GUI_GOTO_NEXT_PAGE.replace("{0}", String.valueOf(gui.getCurrentPage() + 2))
										.replace("{1}", String.valueOf(gui.getPageAmount()))
								: HBLocale.GUI_CANNOT_GOTO_NEXT_PAGE)));
		return builder;
	}
}