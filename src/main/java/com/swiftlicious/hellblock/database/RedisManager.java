...
public class RedisManager extends AbstractStorage {
	...
	public void publishRedisMessage(@NotNull String server, @NotNull String message) {
		...
		HellblockPlugin.getInstance().debug("Sent Redis message: " + message);
		if (isNewerThan5) {
			try (Jedis jedis = jedisPool.getResource()) {
	...
	private void handleMessage(String message) {
		LogUtils.info("Received Redis message: " + message);
	...
	public CompletableFuture<Void> setChangeServer(UUID uuid) {
		...
		HellblockPlugin.getInstance().getScheduler().runTaskAsync(() -> {
			...
			LogUtils.info("Server data set for " + uuid);
		});
		return future;
	}
		...
		HellblockPlugin.getInstance().getScheduler().runTaskAsync(() -> {
			try (Jedis jedis = jedisPool.getResource()) {
				...
				if (jedis.get(key) != null) {
					...
					LogUtils.info("Server data retrieved for " + uuid + "; value: true");
				} else {
					future.complete(false);
					LogUtils.info("Server data retrieved for " + uuid + "; value: false");
				}
			}
		});
		return future;
		...
		HellblockPlugin.getInstance().getScheduler().runTaskAsync(() -> {
			try (Jedis jedis = jedisPool.getResource()) {
				...
				if (data != null) {
					...
					HellblockPlugin.getInstance().debug("Redis data retrieved for " + uuid + "; normal data");
				} else {
					future.complete(Optional.empty());
					HellblockPlugin.getInstance().debug("Redis data retrieved for " + uuid + "; empty data");
				}
			} catch (Exception e) {
				future.complete(Optional.empty());
	...
	public CompletableFuture<Boolean> updatePlayerData(UUID uuid, PlayerData playerData, boolean ignore) {
		...
		HellblockPlugin.getInstance().getScheduler().runTaskAsync(() -> {
			try (Jedis jedis = jedisPool.getResource()) {
				...
				future.complete(true);
				LogUtils.info("Redis data set for " + uuid);
			} catch (Exception e) {
				future.complete(false);
