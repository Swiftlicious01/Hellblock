 ...
 *
 * <p>
 * The "loader" jar contains the loading code & public API classes, and is
 * class-loaded by the platform.
 * </p>
 *
 * <p>
 * The inner "plugin" jar contains the plugin itself, and is class-loaded by the
 * loading code & this classloader.
 * </p>
 */
public class JarInJarClassLoader extends URLClassLoader {
	 ...
	 *
	 * @param loaderClassLoader the loader plugin's classloader (setup and created
	 *                          by the platform)
	 * @param jarResourcePath   the path to the jar-in-jar resource within the
	 *                          loader jar
	 ...
	 * @param loaderPluginType the type of the loader plugin, the only parameter of
	 *                         the bootstrap plugin constructor
	...
	/**
	 * Extracts the "jar-in-jar" from the loader plugin into a temporary file, then
	 * returns a URL that can be used by the {@link JarInJarClassLoader}.
	 *
	...
	private static URL extractJar(ClassLoader loaderClassLoader, String jarResourcePath) throws LoadingException {
		...
		Path path;
		try {
			path = Files.createTempFile("hellblock-jarinjar", ".jar.tmp");
		} catch (IOException e) {
