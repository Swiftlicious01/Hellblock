			...
			if (itemStack.getType() == Material.FISHING_ROD) {
				NBT.get(itemStack, nbtItem -> {
					...
					if (compound != null && compound.hasTag("max_dur")) {
						...
						hook.remove();
						ItemUtils.decreaseDurability(player, NBT.itemStackFromNBT(nbtItem), 2, true);
					}
				});
			}
		}
	...
	private void onCaughtEntity(PlayerFishEvent event) {
		...
		if (player.getGameMode() != GameMode.CREATIVE) {
			...
			if (itemStack.getType() != Material.FISHING_ROD)
				itemStack = player.getInventory().getItemInOffHand();
			NBT.get(itemStack, nbtItem -> {
				ReadableNBT nbtCompound = nbtItem.getCompound("LavaFishing");
				if (nbtCompound != null && nbtCompound.hasTag("max_dur")) {
					event.getHook().remove();
					event.setCancelled(true);
					ItemUtils.decreaseDurability(player, NBT.itemStackFromNBT(nbtItem), 5, true);
				}
			});
		}
	}
