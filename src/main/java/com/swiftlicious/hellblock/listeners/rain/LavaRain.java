...
public class LavaRain {
	...
	public LavaRain(HellblockPlugin plugin) {
		instance = plugin;
		radius = Math.abs(instance.getConfig("config.yml").getInt("lava-rain-options.radius", 16));
		fireChance = Math.abs(instance.getConfig("config.yml").getInt("lava-rain-options.fireChance", 1));
		taskDelay = Math.abs(instance.getConfig("config.yml").getInt("lava-rain-options.taskDelay", 3));
	}

	public void startLavaRainProcess() {
		this.lavaRainTask = new LavaRainTask(instance, true, true, RandomUtils.nextInt(100, 1000));
	}

	public void stopLavaRainProcess() {
		if (this.lavaRainTask != null && this.lavaRainTask.isLavaRaining()) {
