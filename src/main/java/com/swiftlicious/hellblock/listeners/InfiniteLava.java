...
public class InfiniteLava implements Listener {
	...
	private boolean three, two, other, big, plus, tShape, ring, lavaFall;
	private int height;

	public InfiniteLava(HellblockPlugin plugin) {
		instance = plugin;
		this.height = instance.getConfig("config.yml").getInt("infinite-lava-options.height", 60);
		this.three = instance.getConfig("config.yml").getBoolean("infinite-lava-options.sources.three", true);
		this.two = instance.getConfig("config.yml").getBoolean("infinite-lava-options.sources.two", true);
		this.other = instance.getConfig("config.yml").getBoolean("infinite-lava-options.sources.other", false);
		this.big = instance.getConfig("config.yml").getBoolean("infinite-lava-options.sources.big", false);
		this.tShape = instance.getConfig("config.yml").getBoolean("infinite-lava-options.sources.T", true);
		this.ring = instance.getConfig("config.yml").getBoolean("infinite-lava-options.sources.ring", true);
		this.lavaFall = instance.getConfig("config.yml").getBoolean("infinite-lava-options.sources.lava-fall", true);
		Bukkit.getPluginManager().registerEvents(this, instance);
	}

	@EventHandler
	public void onInfiniteLava(BlockFromToEvent event) {
		...
		// if
		// (event.getWorld().getName().equalsIgnoreCase(instance.getWorldHandler().getWorldName()))
		// return;
		if (!(sourceBlock.isLiquid()) || sourceBlock.getType().equals(Material.WATER)) {
			return;
		}

		if (sourceBlock.getY() <= getHeight()) {
			return;
		}
		...
		instance.getScheduler().runTaskSyncLater(() -> {
			...
			if (sourceBlock.getY() > 0 && event.getFace() == BlockFace.DOWN
				...
				if (isLavaFall() && targetBlock.getType() == Material.LAVA
			...
			if (!(sourceBlock.getBlockData() instanceof Levelled)
					|| ((Levelled) sourceBlock.getBlockData()).getLevel() != 0) {
				return;
			}
			...
			if (sourceBlock.getType() == Material.LAVA) {
				if (targetBlock.getType() == Material.LAVA) {
					...
					if (targetLevel.getLevel() != 0 && targetLevel.getLevel() < 8) {
						if (checkLavaSpreadValidity(targetBlock)) {
							targetLevel.setLevel(0);
							targetBlock.setBlockData(targetLevel);
						}
					}
		...
		}, event.getToBlock().getLocation(), 50, TimeUnit.MILLISECONDS);
	}

	public boolean checkLavaSpreadValidity(Block block) {
		/*
		 * NEW Consolidated check code. Returns true if block is fillable, false if
		 * block is not fillable. Our previous methods read several blocks multiple
		 * times and the test logic is duplicated in two places. This processing time
		 * could be better spent elsewhere. Consolidation of these tests allows changes
		 * to be made in one place. checkLavaSpreadValidity is intended to reduce
		 * redundancy and improve efficiency over our previous code.
		 */
		...
		/*
		 * Evaluations Faces and Borders: Faces are full blocks of lava one block N, S,
		 * E, or W of the target. Borders are any solid block (not liquid or air) one
		 * block N, S, E, or W of the target. A diagonal solid is not considered a
		 * border because it would not prevent flow from the target block.
		 */
		for (int i = 0; i <= 3; i++) {
			int v = i * 2;
			...
			/*
			 * Corners and Lake Corners are three contiguous full lava blocks (face,
			 * diagonal, face) and are required to further test for a lake. A Lake is valid
			 * when at least 4 blocks contiguous to the tested corner contain lava of any
			 * amount.
			 */
			cBlocks = 0;
			for (int c = 0; c <= 2; c++) {
		...
		if (faces == 4 && (lBlocks == 0 || lBlocks == 2 || lBlocks == 3 || lBlocks == 4 || lBlocks == 5) && isBig()) {
			// borders and corners are not used here because it they would be redundant and
			// could invalidate a valid
			// fill.
			fill = true;
		}
		// Three, a 3x3 pool. Minimum Requirement: 4 full corners (includes faces)
		else if (borders == 0 && corners == 4 && faces == 4 && !lake && lBlocks == 0 && isThree()) {
			fill = true;
		}
		...
		else if (borders == 2 && corners == 1 && faces == 2 && isTwo()) {
			fill = true;
		}
		...
		else if (borders == 1 && corners == 0 && faces == 3 && isTShape()) {
			fill = true;
		}
