...
import com.swiftlicious.hellblock.gui.YamlPage;
...
public class ItemSelector implements YamlPage {
	...
	public void reOpenWithFilter(String filter) {
		...
		Gui upperGui = Gui.normal().setStructure("a # #")
		...
		var gui = PagedGui.items()
				...
				.addIngredient('c', new BackToFolderItem(file.getParentFile())).setContent(getItemList()).build();

		var window = AnvilWindow
				.split().setViewer(player).setTitle(new ShadedAdventureComponentWrapper(HellblockPlugin.getInstance()
						.getAdventureManager().getComponentFromMiniMessage(HBLocale.GUI_SELECT_ITEM)))
				.addRenameHandler(s -> {
	...
	public void reOpenWithNewKey() {
		String tempKey = HBLocale.GUI_TEMP_NEW_KEY;
		prefix = tempKey;
		...
		var gui = PagedGui.items()
				...
				.addIngredient('x', Markers.CONTENT_LIST_SLOT_HORIZONTAL).addIngredient('#', new BackGroundItem())
				.addIngredient('a', new PreviousPageItem()).addIngredient('b', new NextPageItem())
				.addIngredient('c', new BackToFolderItem(file.getParentFile())).build();

		var window = AnvilWindow
				.split().setViewer(player).setTitle(new ShadedAdventureComponentWrapper(HellblockPlugin.getInstance()
						.getAdventureManager().getComponentFromMiniMessage(HBLocale.GUI_SET_NEW_KEY)))
				.addRenameHandler(s -> {
					long current = System.currentTimeMillis();
					if (current - coolDown < 100)
						return;
	...
	public class AddKey extends AbstractItem {
		...
		public ItemProvider getItemProvider() {
			return new ItemBuilder(Material.ANVIL).setDisplayName(new ShadedAdventureComponentWrapper(HellblockPlugin
					.getInstance().getAdventureManager().getComponentFromMiniMessage(HBLocale.GUI_ADD_NEW_KEY)));
		}
	...
	public class ConfirmIcon extends AbstractItem {
		...
		public ItemProvider getItemProvider() {
			if (prefix != null && !yaml.contains(prefix) && prefix.matches("^[a-zA-Z0-9_]+$")) {
				var builder = new ItemBuilder(Material.NAME_TAG).setDisplayName(new ShadedAdventureComponentWrapper(
				...
				return new ItemBuilder(Material.BARRIER)
						.setDisplayName(new ShadedAdventureComponentWrapper(HellblockPlugin.getInstance()
								.getAdventureManager().getComponentFromMiniMessage(HBLocale.GUI_DUPE_INVALID_KEY)));
			}
		}
