...
public class EffectManager implements EffectManagerInterface {
	...
	public BaseEffect getBaseEffect(ConfigurationSection section) {
		if (section == null)
			return new BaseEffect(new PlainValue(0), new PlainValue(1d), new PlainValue(0), new PlainValue(1d));
		Value waitTime = section.contains("wait-time") ? instance.getConfigUtils().getValue(section.get("wait-time"))
				: new PlainValue(0);
		Value difficulty = section.contains("difficulty")
				? instance.getConfigUtils().getValue(section.get("difficulty"))
				: new PlainValue(0);
		Value waitTimeMultiplier = section.contains("wait-time-multiplier")
				...
				: new PlainValue(1);
		Value difficultyMultiplier = section.contains("difficulty-multiplier")
				? instance.getConfigUtils().getValue(section.get("difficulty-multiplier"))
				: new PlainValue(1);
		return new BaseEffect(waitTime, waitTimeMultiplier, difficulty, difficultyMultiplier);
	}
	...
	public EffectModifier getEffectModifier(ConfigurationSection section) {
		...
		case "difficulty-multiplier", "difficulty-bonus" -> {
			var value = instance.getConfigUtils().getValue(section.get("value"));
			return ((effect, condition) -> {
				effect.setDifficultyMultiplier(
						effect.getDifficultyMultiplier() + value.get(condition.getPlayer(), condition.getArgs()) - 1);
			});
		}
		case "multiple-loot" -> {
			var value = instance.getConfigUtils().getValue(section.get("value"));
