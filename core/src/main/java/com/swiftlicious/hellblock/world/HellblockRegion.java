...
public class HellblockRegion implements HellblockRegionInterface {
	...
	protected HellblockRegion(HellblockWorld<?> world, RegionPos regionPos) {
		this.world = world;
	...
	protected HellblockRegion(HellblockWorld<?> world, RegionPos regionPos,
			ConcurrentMap<ChunkPos, byte[]> cachedChunks) {
		this.world = world;
		this.regionPos = regionPos;
	...
	public void unload() {
		if (this.isLoaded) {
			if (((HellblockWorld<?>) world).unloadRegion(this)) {
				this.isLoaded = false;
			}
		}
	}
	...
	public void load() {
		if (!this.isLoaded) {
			if (((HellblockWorld<?>) world).loadRegion(this)) {
				this.isLoaded = true;
			}
		}
	}
