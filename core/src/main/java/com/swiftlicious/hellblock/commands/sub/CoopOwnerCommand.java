		...
		return builder.senderType(Player.class)
				.required("player", StringParser.stringComponent().suggestionProvider(new SuggestionProvider<>() {
					...
					if (data.getTransferCooldown() > 0) {
						handleFeedback(context, MessageConstants.MSG_HELLBLOCK_TRANSFER_ON_COOLDOWN.arguments(Component
								.text(HellblockPlugin.getInstance().getFormattedCooldown(data.getTransferCooldown()))));
						return;
					}
					...
					HellblockPlugin.getInstance().getStorageManager()
							...
							.thenAccept(result -> {
								...
								try {
									...
									HellblockPlugin.getInstance().getCoopManager().transferOwnershipOfHellblock(user,
											targetUser, false);
								} catch (Exception ex) {
									HellblockPlugin.getInstance().getPluginLogger()
											.warn("transferOwnershipOfHellblock failed from " + player.getName()
													+ " to " + targetName + ": " + ex.getMessage());
									return;
								}
							}).exceptionally(ex -> {
								HellblockPlugin.getInstance().getPluginLogger()
										.warn("getOfflineUserData failed for owner transfership of " + targetName + ": "
												+ ex.getMessage());
								return null;
							});
				});
