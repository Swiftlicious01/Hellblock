...
public class HellblockHelpCommand extends BukkitCommandFeature<CommandSender> {
	...
	public Command.Builder<? extends CommandSender> assembleCommand(CommandManager<CommandSender> manager,
		...
		return builder.senderType(Player.class)
				.optional("page", StringParser.stringComponent().suggestionProvider((context, input) -> {
					...
					List<String> lines = getHelpLines();
					...
					try {
						...
						int totalPages = Math.max(1,
								(int) Math.ceil(getHelpLines().size() / (double) ENTRIES_PER_PAGE));
					...
					handleFeedback(context, MessageConstants.MSG_HELLBLOCK_USER_HELP_HEADER,
							AdventureHelper.miniMessageToComponent(String.valueOf(page)),
							AdventureHelper.miniMessageToComponent(String.valueOf(totalPages)));
	...
	private List<String> getHelpLines() {
		return plugin.getTranslationManager().getRawStringList(MessageConstants.MSG_HELLBLOCK_USER_HELP.build().key());
	}
