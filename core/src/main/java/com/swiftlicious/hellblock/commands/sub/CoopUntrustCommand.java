		...
		return builder.senderType(Player.class)
				.required("player", StringParser.stringComponent().suggestionProvider(new SuggestionProvider<>() {
					...
					public @NotNull CompletableFuture<? extends @NotNull Iterable<? extends @NotNull Suggestion>> suggestionsFuture(
						...
						// Suggest trusted players that are not in party and not self
					...
					HellblockPlugin.getInstance().getStorageManager()
							...
							.thenAccept(result -> {
								...
								targetUser.getHellblockData().removeTrustPermission(player.getUniqueId());

								HellblockPlugin.getInstance().getCoopManager()
										.removeTrustAccess(user, targetName, targetId).thenAccept(trust -> {
											...
											HellblockPlugin.getInstance().getPluginLogger()
													.warn("removeTrustAccess failed for " + targetName + ": "
															+ ex.getMessage());
											return null;
										});
							}).exceptionally(ex -> {
								HellblockPlugin.getInstance().getPluginLogger()
										.warn("getOfflineUserData failed for revoke trust of " + targetName + ": "
												+ ex.getMessage());
								return null;
							});
				});
