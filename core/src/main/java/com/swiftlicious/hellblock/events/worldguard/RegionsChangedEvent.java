package com.swiftlicious.hellblock.events.worldguard;
...
public class RegionsChangedEvent extends Event implements Cancellable {
	...
	/**
	 * This event is fired whenever a region is entered. It may be fired multiple
	 * times per tick, if several regions are entered at the same time.
	 * 
	 * @param playerUUID The UUID of the player entering the region.
	 * @param previous   Set of WorldGuard's ProtectedRegion the player was in
	 *                   before this event
	 * @param current    Set of WorldGUard's ProtectedRegion the player is currently
	 *                   in
	 */
	public RegionsChangedEvent(UUID playerUUID, @NotNull Set<ProtectedRegion> previous,
			@NotNull Set<ProtectedRegion> current) {
