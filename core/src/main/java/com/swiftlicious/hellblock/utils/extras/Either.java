...
/**
 * Interface representing a value that can be either of two types, primary or
 * fallback. Provides methods to create instances of either type and to map
 * between them.
 *
...
public interface Either<U, V> {
	 ...
	 *
	 * @return an {@link Optional} containing the primary value, or empty if not
	 *         present
	 */
	@NotNull
	Optional<U> primary();
	 ...
	 *
	 * @return an {@link Optional} containing the fallback value, or empty if not
	 *         present
	 */
	@NotNull
	Optional<V> fallback();
	...
	/**
	 * Retrieves the primary value, or maps the fallback value to the primary type
	 * if the primary is not present.
	 *
	 * @param mapFallback a function to map the fallback value to the primary type
	 * @return the primary value, or the mapped fallback value if the primary is not
	 *         present
	 */
	...
	/**
	 * Retrieves the fallback value, or maps the primary value to the fallback type
	 * if the fallback is not present.
	 *
	 * @param mapPrimary a function to map the primary value to the fallback type
	 * @return the fallback value, or the mapped primary value if the fallback is
	 *         not present
	 */
	 ...
	 */
	default @NotNull <R> R mapEither(final @NotNull Function<U, R> mapPrimary,
