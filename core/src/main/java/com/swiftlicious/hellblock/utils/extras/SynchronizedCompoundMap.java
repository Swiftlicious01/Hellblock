...
public class SynchronizedCompoundMap {
	...
	/**
	 * Puts a Tag into the map with the specified key, returning the previous value
	...
	private String compoundMapToString(String key, CompoundMap compoundMap) {
		...
		for (Map.Entry<String, Tag<?>> entry : compoundMap.entrySet()) {
			...
			String tagValue;
			switch (tag.getType()) {
			case TAG_STRING, TAG_BYTE, TAG_DOUBLE, TAG_FLOAT, TAG_INT, TAG_INT_ARRAY, TAG_LONG, TAG_SHORT,
					TAG_SHORT_ARRAY, TAG_LONG_ARRAY, TAG_BYTE_ARRAY ->
