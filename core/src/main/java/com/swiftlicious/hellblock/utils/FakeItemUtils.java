...
public class FakeItemUtils {
	...
	/**
	 * Creates a teleport packet for moving a fake item entity to the specified
	 * location.
	 *
	 ...
	 */
	public static PacketContainer getTpPacket(int id, Location location, boolean onGround) {
		...
		tpPacket.getDoubles().write(1, location.getY() - 0.6);
		tpPacket.getDoubles().write(2, location.getZ());
		tpPacket.getBytes().write(0, (byte) yaw);
		tpPacket.getBytes().write(1, (byte) pitch);
		tpPacket.getBooleans().write(0, onGround);
		return tpPacket;
	}
