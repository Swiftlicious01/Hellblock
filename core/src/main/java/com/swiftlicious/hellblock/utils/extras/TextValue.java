...
/**
 * The TextValue interface represents a text value that can be rendered within a
 * specific context. This interface allows for the rendering of
 * placeholder-based or plain text values in the context of custom mechanics.
 *
...
public interface TextValue<T> {
	...
	/**
	 * Creates a TextValue based on a placeholder text. Placeholders can be
	 * dynamically replaced with context-specific values.
	 *
	...
	static <T> TextValue<T> placeholder(String text) {
		return new PlaceholderTextValue<>(text);
	}
	...
	static <T> TextValue<T> plain(String text) {
		return new PlainTextValue<>(text);
	}
	 ...
	 * @param <T> the type of the holder object for the context
	 * @return a TextValue instance representing the given text, either as a
