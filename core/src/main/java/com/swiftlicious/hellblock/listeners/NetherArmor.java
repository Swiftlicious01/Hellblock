...
public class NetherArmor implements Listener, Reloadable {
	...
	public void addArmor() {
		try {
			Function<Object, BiConsumer<Item<ItemStack>, Context<Player>>> f1 = arg -> {
				return (item, context) -> {
					for (Entry<NamespacedKey, ShapedRecipe> recipe : registerNetherArmor(context,
	...
	private Map<NamespacedKey, ShapedRecipe> registerNetherArmor(Context<Player> context, Section section) {
		...
		for (Map.Entry<String, Object> entry : section.getStringRouteMappedValues(false).entrySet()) {
			if (entry.getValue() instanceof Section inner) {
				...
				armorMap.putIfAbsent(Material.getMaterial(inner.getString("material")),
