...
public class LavaFishingMechanic implements HookMechanic {
	...
	public boolean canStart() {
		...
		FluidData fluidData = this.hook.getWorld().getFluidData(location);
	...
	public boolean shouldStop() {
		...
		return hook.isOnGround() || (hook.getLocation().getBlock().getType() != Material.LAVA
				&& hook.getLocation().getBlock().getRelative(BlockFace.DOWN).getType() != Material.LAVA);
	}
	...
	public void start(Effect finalEffect) {
		...
		this.task = HellblockPlugin.getInstance().getScheduler().sync().runRepeating(() -> {
			Location location = this.hook.getLocation();
			...
			if (this.nibble > 0) {
				...
				if (hookY % 1 <= lavaHeight || this.hook.isInLava()) {
					...
					if (currentState == 1) {
						this.currentState = 0;
						// set temp entity
						this.tempEntity = this.hook.getWorld().spawn(location.clone().subtract(0, 1, 0),
								ArmorStand.class);
						this.setTempEntityProperties(this.tempEntity);
						this.hook.setHookedEntity(this.tempEntity);
						if (!firstTime) {
							EventUtils.fireAndForget(new FishingHookStateEvent(context.holder(), hook,
									FishingHookStateEvent.State.ESCAPE));
						}
						firstTime = false;
					}
				}
				...
				if (this.timeUntilHooked > 0) {
					...
					if (this.timeUntilHooked > 0) {
						...
						if (RandomUtils.generateRandomFloat(0, 1) < 0.15F) {
							hook.getWorld().spawnParticle(Particle.FLAME, d0, d1 - 0.10000000149011612D, d2, 1, f1,
									0.1D, f2, 0.0D);
						}
						float f3 = f1 * 0.04F;
						float f4 = f2 * 0.04F;
						...
						hook.getWorld().spawnParticle(Particle.FLAME, location.getX(), d3, location.getZ(),
					...
					if (this.timeUntilLured <= 0) {
						...
						EventUtils.fireAndForget(
								new FishingHookStateEvent(context.holder(), hook, FishingHookStateEvent.State.LURE));
					}
				} else {
	...
	private void setTempEntityProperties(ArmorStand entity) {
		...
		entity.getPersistentDataContainer().set(
				Objects.requireNonNull(NamespacedKey.fromString("temp-entity", HellblockPlugin.getInstance())),
				PersistentDataType.STRING, "lava");
	}
