...
import com.swiftlicious.hellblock.mechanics.hook.HookConfigInterface;
...
public class HookConfigParser {
	...
	private void analyze(Section section, Map<String, Node<ConfigParserFunction>> functionMap) {
		...
		for (Map.Entry<String, Object> entry : dataMap.entrySet()) {
			...
			if (function != null) {
				...
				case HOOK -> {
					final HookParserFunction hookParserFunction = (HookParserFunction) function;
					final Consumer<HookConfig.Builder> consumer = hookParserFunction.accept(entry.getValue());
	...
	public EventCarrier getEventCarrier() {
		final EventCarrier.Builder builder = EventCarrierInterface.builder().id(id).type(MechanicType.HOOK);
		eventBuilderConsumers.forEach(consumer -> consumer.accept(builder));
		return builder.build();
	}

	public EffectModifier getEffectModifier() {
		final EffectModifier.Builder builder = EffectModifierInterface.builder().id(id).type(MechanicType.HOOK);
		effectBuilderConsumers.forEach(consumer -> consumer.accept(builder));
		return builder.build();
	}

	public HookConfig getHook() {
		final HookConfig.Builder builder = HookConfigInterface.builder().maxUsages(maxUsages).id(id);
		hookBuilderConsumers.forEach(consumer -> consumer.accept(builder));
		return builder.build();
	}
