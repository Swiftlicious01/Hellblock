...
/**
 * The ActionManager interface manages custom action types and provides methods
 * for handling actions.
 *
 * @param <T> the type of the context in which the actions are triggered.
 */
public interface ActionManagerInterface<T> extends Reloadable {

	/**
	 * Registers a custom action type with its corresponding factory.
	 *
	 * @param actionFactory The factory responsible for creating instances of the
	 *                      action.
	 ...
	 *
	 * @param type The type identifier of the action.
	 * @return The action factory for the specified type, or null if no factory is
	 *         found.
	 */
	@Nullable
	...
	/**
	 * Generates a map of actions triggered by specific events from a configuration
	 * section.
	 *
	 * @param section The configuration section containing event-action mappings.
	 * @return A map where the keys are action triggers and the values are arrays of
	 ...
	 * @return A TreeMap where the keys are time values (in integer form) and the
	...
	/**
	 * Triggers a list of actions with the given context. If the list of actions is
	 * not null, each action in the list is triggered.
	 *
	...
	/**
	 * Triggers an array of actions with the given context. If the array of actions
	 * is not null, each action in the array is triggered.
	 *
	 * @param context The context associated with the actions.
