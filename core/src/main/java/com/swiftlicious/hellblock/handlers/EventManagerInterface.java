...
/**
 * EventManager interface for managing events and their associated actions. It
 * provides methods to register, retrieve, and trigger events based on different
 * conditions.
 */
public interface EventManagerInterface extends Reloadable {
	 ...
	 * @param type
	 * @return An Optional containing the EventCarrier if found, or an empty
	...
	/**
	 * Triggers an event for a given context, identifier, trigger, and a range of
	 * times.
	 *
	 * @param context       The context in which the event is triggered.
	 * @param id            The unique identifier of the event carrier.
	 ...
	 */
	default void trigger(Context<Player> context, String id, MechanicType type, ActionTrigger trigger,
			int previousTimes, int afterTimes) {
		getEventCarrier(id, type).ifPresent(carrier -> trigger(context, carrier, trigger, previousTimes, afterTimes));
	}

	/**
	 * Triggers the event actions for a given context and trigger on a specified
	 * carrier.
	 *
	 * @param context The context in which the event is triggered.
	...
	/**
	 * Triggers the event actions for a given context, trigger, and times range on a
	 * specified carrier.
	 *
	 * @param context       The context in which the event is triggered.
	 * @param carrier       The event carrier.
	 ...
	 * @param afterTimes    The after times count for the event.
	 */
	static void trigger(Context<Player> context, EventCarrier carrier, ActionTrigger trigger, int previousTimes,
			int afterTimes) {
		if (!carrier.disableGlobalActions()) {
	...
	/**
	 * Triggers global timed actions for a given context, item type, trigger, and
	 * times range.
	 *
	 * @param context       The context in which the event is triggered.
	 * @param type          The type of item that triggered the event.
